...
//        grid3DRenderer = new DummyGrid3DRenderer();
        ...
        var texture1 = new Texture( "resources/First Oracle/texture3D.png" );
        var texture2 = new Texture( "resources/First Oracle/texture3Dc.png" );
        var texture3 = new Texture( "resources/First Oracle/texture3DHEX.png" );
        var texture4 = new Texture( "resources/First Oracle/texture3DHEX2.png" );
//        texture1 = texture2 = texture3 = texture4 = FirstOracleConstants.EMPTY_TEXTURE;
//        Texture compoundTexture = Texture.createCompoundTexture( "resources/First Oracle/compound/tex_#frame#_#direction#.png",
//            4,
//            6
//        );
        var cube1 = new NonAnimatedPlane3D();
        cube1.setTexture( texture1 );
        cube1.getTransformations().setPosition( 0, 0, 0 );
    
        var cube2 = new NonAnimatedPlane3D();
        cube2.setTexture( texture2 );
        cube2.getTransformations().setPosition( 4, 0, 0 );
    
        var hex1 = new NonAnimatedPlane3D();
        hex1.setTexture( texture3 );
        hex1.getTransformations().setPosition( 8, 0, 0 );
    
        var hex2 = new NonAnimatedPlane3D();
        hex2.setTexture( texture4 );
        hex2.getTransformations().setPosition( 12, 0, 0 );
    
        var hex3 = new NonAnimatedPlane3D();
...
//        compound.getTransformations().setScale( 4, 4 );
        ...
        var array = new RectangleGrid[ 0 ][ 0 ];
    
        var grid = new NonAnimatedCubeGrid();
        grid.setTexture( texture1 );
    
        var terrain3DS = new NonAnimatedCubeGrid[ 0 ][ 0 ][ 0 ];
        for ( var x = 0; x < terrain3DS.length; x++ ) {
            for ( var y = 0; y < terrain3DS[ x ].length; y++ ) {
                for ( var z = 0; z < terrain3DS[ x ][ y ].length; z++ ) {
...
//            Collections.emptyList();
            Arrays.asList( cube1, cube2, hex1, hex2, hex3 );
        
        renderedScene.setScene3D( (
            new RenderedScene3D() {
                @Override

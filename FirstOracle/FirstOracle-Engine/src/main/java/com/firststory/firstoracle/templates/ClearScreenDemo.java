...
import static com.firststory.firstoracle.templates.VKUtil.translateVulkanResult;
...
public class ClearScreenDemo {
    ...
    private static VkInstance createInstance(PointerBuffer requiredExtensions) {
        ...
        // This includes stuff like the Window System Interface extensions to actually loopInside something on a window.
        //
    ...
    private static long createClearRenderPass(VkDevice device, int colorFormat) {
        ...
        attachments.free();
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to create clear loopInside pass: " + translateVulkanResult(err));
        }
        return renderPass;
    }
    ...
    private static VkCommandBuffer[] createRenderCommandBuffers(VkDevice device, long commandPool, long[] framebuffers, long renderPass, int width, int height) {
        // Create the loopInside command buffers (one command buffer per framebuffer image)
        VkCommandBufferAllocateInfo cmdBufAllocateInfo = VkCommandBufferAllocateInfo.calloc()
            .sType(VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO)
            .commandPool(commandPool)
            .level(VK_COMMAND_BUFFER_LEVEL_PRIMARY)
        ...
        int err = vkAllocateCommandBuffers(device, cmdBufAllocateInfo, pCommandBuffer);
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to allocate loopInside command buffer: " + translateVulkanResult(err));
        }
        ...
        // Specify everything to begin a loopInside pass
        ...
        for (int i = 0; i < renderCommandBuffers.length; ++i) {
            ...
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to begin loopInside command buffer: " + translateVulkanResult(err));
            }
    ...
    public static void main(String[] args) {
        ...
        final class SwapchainRecreator {
            ...
            void recreate() {
                ...
                // Create loopInside command buffers
        ...
        // Pre-allocate everything needed in the loopInside loop
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            // Create a semaphore to wait for the loopInside to complete, before presenting
            err = vkCreateSemaphore(device, semaphoreCreateInfo, null, pRenderCompleteSemaphore);
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to create loopInside complete semaphore: " + translateVulkanResult(err));
            }
            ...
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to submit loopInside queue: " + translateVulkanResult(err));
            }

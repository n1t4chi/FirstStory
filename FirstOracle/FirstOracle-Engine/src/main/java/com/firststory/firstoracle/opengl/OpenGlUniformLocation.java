...
class OpenGlUniformLocation {
    ...
    void bind( Colour colour ) {
        if ( !colour.equals( lastBind ) ) {
            GL20.glUniform4f( locationID, colour.red(), colour.green(), colour.blue(), colour.alpha() );
            lastBind = colour;
        }
    }
    
    void bind( Position position ) {
        if ( !position.equals( lastBind ) ) {
            GL20.glUniform3f( locationID, position.x(), position.y(), position.z() );
            lastBind = position;
        }
    }
    
    void bind( Rotation rotation ) {
        if ( !rotation.equals( lastBind ) ) {
            GL20.glUniform3f( locationID, rotation.ox(), rotation.oy(), rotation.oz() );
            lastBind = rotation;
        }
    }
    
    void bind( Scale scale ) {
        if ( !scale.equals( lastBind ) ) {
            GL20.glUniform3f( locationID, scale.x(), scale.y(), scale.z() );
            lastBind = scale;
        }
    }
    
    void bind( UV uv ) {
        if ( !uv.equals( lastBind ) ) {
            GL20.glUniform2f( locationID, uv.u(), uv.v() );
            lastBind = uv;
        }
    }

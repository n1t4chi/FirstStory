...
import com.firststory.firstoracle.IOUtilities;
...
public final class Texture implements Closeable{
     ...
     */
    public Texture(BufferedImage image, int frames,int directions, int rows,int columns) throws IOException {
     ...
     * @param bf Buffer where texture data is stored.
     ...
     * @throws IOException 
     */
    public Texture(ByteBuffer bf,String name, int frames,int directions, int rows,int columns) throws IOException {
        if(name == null || name.isEmpty() || frames<1 || rows<1 || frames>rows || directions<1 || directions>columns)
     ...
     * @throws IOException 
     */
    public Texture(String path, int frames,int directions, int rows,int columns) throws IOException {
    ...
    /**
     * Releases texture resources associated with this object by calling {@link #releaseTexture()}
     */
    @Override
    public void close(){
        releaseTexture();
     ...
     * Use {@link #bindTexture() } for reusable texture.
     */
